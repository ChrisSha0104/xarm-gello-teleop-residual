...
def plot_two_depth_histograms(filepath1, filepath2, bins=15):
    ...
    selected_depths1 = all_depths1[(all_depths1 >= 0.1) & (all_depths1 <= 0.5)]
    selected_depths2 = all_depths2[(all_depths2 >= 0.1) & (all_depths2 <= 0.5)]

    print(f"Selected points: {selected_depths1.shape[0]} (dataset 1), {selected_depths2.shape[0]} (dataset 2)")

    # Plot
    plt.figure(figsize=(8, 6))
    plt.hist(selected_depths1, bins=bins, range=(0.1, 0.5), alpha=0.5, label='Sim', edgecolor='black')
    plt.hist(selected_depths2, bins=bins, range=(0.1, 0.5), alpha=0.5, label='Real', edgecolor='black')
...
if __name__ == "__main__":
    ...
    plot_two_depth_histograms('experiments/real_world/modules_teleop/RRL/tasks/cube/sim2real/traj2/sim_depth_obs.npy', 'experiments/real_world/modules_teleop/RRL/tasks/cube/sim2real/traj2/real_depth_obs.npy', bins=30)
